-- ══════════════════════════════════════════════════════════════════════════════
-- PonteGuapa GT — Schema completo v2
-- Actualización: se agrega promocion_id en citas para vincular la promoción usada
--
...
-- la especialidad requerida para filtrar qué estilistas pueden realizarlo.
-- ─────────────────────────────────────────────────────────────────────────────
...
-- promocion_id registra qué promoción se aplicó al agendar (nullable).
...
CREATE TABLE IF NOT EXISTS citas (
  ...
  CONSTRAINT fk_cita_promo     FOREIGN KEY (promocion_id) REFERENCES promociones(id) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
--   · citas.promocion_id INT NULL + FK → promociones ON DELETE SET NULL
--   · resenas.puntuacion CHECK (puntuacion BETWEEN 1 AND 5)
-- Tablas (16):
...
--   notificaciones, notificaciones_estilista, resenas
-- ══════════════════════════════════════════════════════════════════════════════
